...
function Loader() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setBrightness((prevState) => prevState + Math.PI / 10);
    }, 50);

    return () => clearInterval(interval);
  }, []);

  const calculateBrightness = (state: number) => Math.floor(((Math.sin(state) + 1) * 49));
  const calculatedBrightness = calculateBrightness(brightness).toString(10);
  ...
  return (
    <svg width="150" height="170" viewBox="0 5 14 34" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        fill={`#${preparedBrightness}${preparedBrightness}${preparedBrightness}`}
      />
    </svg>
  );
}
